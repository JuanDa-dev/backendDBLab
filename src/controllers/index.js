...
export const getContinents = async (req, res) => {
    try {
        ...
        FROM countries c GROUP BY c.continent`)
        res.json(rows)
    } catch (err) {
        httpError(res)
    }
...
export const getDataContinent = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('SELECT * FROM countries c, datacountries dc WHERE dc.idCountry = c.isoCode and c.continent = ? ORDER BY dc.dateR ASC', [continent])
        res.json(rows)
    } catch (err) {
        httpError(res)
    }
...
export const getDataCountry = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query('SELECT * FROM countries c, datacountries dc WHERE c.isoCode = dc.idCountry and c.isoCode = ? ORDER BY dc.dateR ASC', [isoCode])
        res.json(rows)
    } catch (err) {
        httpError(res)
    }
